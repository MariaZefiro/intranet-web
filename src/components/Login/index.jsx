...
const Login = () => {
    ...
    const nomeCompletoFromUrl = params.get('nome_completo') || '';
    const [login, setLogin] = useState(usernameFromUrl);
    const [password, setPassword] = useState(nomeCompletoFromUrl);
    ...
    const handleLogin = async (e) => {
        e.preventDefault();
        try {
            const { data } = await axios.post(`${backendIp}/api/login`, {
                username: login,
                nome_completo: password,
            });

            if (data.success) {
                localStorage.setItem('username', login);
                
                // Verifica se o usuário é "Comunicação" e define isAdmin
                if (login === "Comunicacao") {
                    localStorage.setItem('isAdmin', true);
                } else {
                    localStorage.removeItem('isAdmin');
                }

                if (login === "Comunicacao" || login === "TI") {
                    localStorage.setItem('isLogged', true);
                    localStorage.setItem('nome_completo', login);
                } else {
                    localStorage.setItem('isLogged2', login);
